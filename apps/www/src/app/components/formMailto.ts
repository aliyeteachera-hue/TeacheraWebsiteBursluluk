...
function getProxyEndpoint() {
  const configured = normalizeConfiguredValue(import.meta.env.VITE_FORMS_PROXY_ENDPOINT || '');
  if (configured) return configured;
  return resolveExamEndpoint('/api/forms');
}
...
export async function openMailDraft({ to = 'data@teachera.com.tr', subject, lines }: MailDraftOptions): Promise<boolean> {
  ...
  try {
    ...
    endpointDomain = new URL(proxyEndpoint, window.location.origin).hostname;
  } catch (error) {
    trackEvent('lead_form_submit_failure', {
      ...
      error_message: error instanceof Error ? error.message.slice(0, 120) : 'proxy_endpoint_not_configured',
    });
    return false;
  }
...
export function openMailDraftOnUnload({ to = 'data@teachera.com.tr', subject, lines }: MailDraftOptions): boolean {
  ...
  try {
    ...
    endpointDomain = new URL(endpoint, window.location.origin).hostname;
  } catch (error) {
    trackEvent('lead_form_submit_failure', {
      form_subject: subjectKey,
      form_id: `lead_${subjectKey}`,
