...
function readSameSiteValue() {
  const value = safeTrim(process.env.PANEL_SESSION_COOKIE_SAMESITE || 'Strict').toLowerCase();
...
export function readPanelSessionIdleTimeoutMinutes() {
  const ttlMinutes = readPanelSessionTtlMinutes();
  const fallback = Math.min(DEFAULT_IDLE_TIMEOUT_MINUTES, ttlMinutes);
  return readBoundedIntEnv('PANEL_SESSION_IDLE_TIMEOUT_MINUTES', fallback, 5, ttlMinutes);
}

export function readPanelMaxActiveSessions() {
  return readBoundedIntEnv('PANEL_MAX_ACTIVE_SESSIONS', 3, 1, 20);
}
...
function shouldUseSecureCookies(req) {
  const forcedSecure = safeTrim(process.env.PANEL_SESSION_COOKIE_SECURE).toLowerCase();
  if (forcedSecure) {
    return ['1', 'true', 'yes', 'on'].includes(forcedSecure);
  }
...
export function buildPanelSessionCookie(token, req, maxAgeSeconds) {
  ...
  const sameSite = readSameSiteValue();
  if (sameSite === 'None' && !secure) {
    throw new HttpError(
      500,
      'SameSite=None requires Secure panel session cookies.',
      'invalid_panel_cookie_policy',
    );
  }
